import os
from pathlib import Path

import pytest

from knowledge_base.arango import ArangoClient
from knowledge_base.config import load_settings
from knowledge_base.indexing import rebuild_indexes
from knowledge_base.repository import KnowledgeRepository
from knowledge_base.retrieval import graph_neighbors, hybrid_search, text_search
from knowledge_base.schema import bootstrap_schema
from knowledge_base.sources.book_cube import ingest_book_cube

pytestmark = pytest.mark.integration


HTML_FIXTURE = Path("tests/fixtures/book_cube_channel.html")
JSON_FIXTURE = Path("tests/fixtures/book_cube_export.json")


def _integration_enabled() -> bool:
    return os.getenv("KB_RUN_INTEGRATION") == "1"


@pytest.mark.skipif(not _integration_enabled(), reason="set KB_RUN_INTEGRATION=1 with ArangoDB running")
def test_book_cube_ingest_end_to_end() -> None:
    settings = load_settings()
    client = ArangoClient(settings)
    repository = KnowledgeRepository(client)

    bootstrap_schema(client)
    ingest_result = ingest_book_cube(repository, settings, input_path=HTML_FIXTURE)
    dedupe_result = ingest_book_cube(repository, settings, input_path=HTML_FIXTURE)
    json_result = ingest_book_cube(repository, settings, input_path=JSON_FIXTURE)
    index_result = rebuild_indexes(repository, target="all")
    text = text_search(repository, "системное чтение")
    graph = graph_neighbors(repository, topic="books")
    hybrid = hybrid_search(repository, "книги исследовательские карточки", dimension=settings.embedding_dimension)

    assert ingest_result["status"] == "ok"
    assert ingest_result["source_key"] == "book-cube"
    assert ingest_result["input"]["kind"] == "file"
    assert ingest_result["skipped"] == [{"guid": "book_cube/103", "reason": "empty_text"}]
    assert dedupe_result["created"]["documents"] == 0
    assert dedupe_result["created"]["chunks"] == 0
    assert json_result["status"] == "ok"
    assert json_result["skipped"] == [{"guid": "202", "reason": "unsupported_type"}]
    assert index_result["status"] == "ok"
    assert text["results"]
    assert graph["status"] == "ok"
    assert graph["results"]
    assert hybrid["status"] in {"ok", "degraded"}
    assert hybrid["results"]
    _assert_book_cube_provenance(text["results"])
    _assert_book_cube_provenance(graph["results"])
    _assert_book_cube_provenance(hybrid["results"])


def _assert_book_cube_provenance(results: list[dict]) -> None:
    assert any(result["provenance"]["source_key"] == "book-cube" for result in results)
    for result in results:
        if result["provenance"]["source_key"] == "book-cube":
            assert result["provenance"]["raw_snapshot_key"]
            assert result["provenance"]["import_run_key"]
