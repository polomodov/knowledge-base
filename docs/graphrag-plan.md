...
| GR-3c | `feat/graph-candidate-expansion` | GR-3 (recall) | GR-3d + GR-2b (реальные эмбеддинги) | 🟡 на ревью |
...
**Реализация (принято):** отдельный index-шаг `kb index rebuild --target related` → `build_related_edges`. Для каждого чанка берётся top-`RELATED_TOP_K` соседей с косинусом ≥ `RELATED_MIN_SCORE` из ДРУГИХ документов и той же embedding-модели (несовместимая модель той же размерности = чужое пространство, GR-2). Рёбра неориентированные (один на пару, детерминированный ключ), с `weight` и `method="embedding-similarity"`; запись идемпотентна (батч-UPSERT одним AQL). **Масштабирование (важно, реальный корпус ~25k чанков):** полный проход использует ANN-индекс (`APPROX_NEAR_COSINE`) — не O(N²); есть scoped-режим `source_key` (сравнение внутри одного источника, изолированно — им же пользуется интеграционный тест, не трогая весь корпус). Чистое ядро отбора (`_select_related`, `_scored_candidates`) покрыто unit-тестами; `_cosine` вынесен в `embeddings.cosine_similarity`. **`--target related` НЕ входит в `all`** — операция O(N²)/ANN-тяжёлая, запускается осознанно.
