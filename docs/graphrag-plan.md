...
|----:|------------|----------|-----------|--------|
| GR-0 | `docs/graphrag-plan` | GR-6 (доки) | — | 🟡 на ревью |
| GR-1 | `feat/graph-aware-hybrid` | GR-1 | GR-0 | ☐ не начат |
| GR-2 | `feat/pluggable-embeddings` | GR-2 | — | ☐ не начат |
...
**Проблемы:** GR-1
**Задача:** Ввести реальный вклад графа в ранжирование `hybrid_search`. После слияния BM25+вектор расширять top-k кандидатов обходом графа (`1..2 ANY`) и подмешивать буст за связность: общие `topic`/`author`/`work` с другими сильными кандидатами, близость по путям, будущие рёбра `item_related_to_item` (после GR-3). Заполнить `score_components.graph_boost` реальным числом вместо `None` и включить его в итоговый скор.
...
**Открытое решение:** локальная модель (`sentence-transformers`, +зависимость, приватно) vs внешний API (без веса, но данные уходят наружу — для личного корпуса нежелательно) vs гибрид. Рекомендация: локальная модель за optional-extra, дефолт — hash.
