...
- **Контракт:** оба возвращают `{query, mode: "graphrag-local"|"graphrag-global", status, degraded_components, …}`; при runtime/AQL failure графового слоя — `status="degraded"`, а не исключение. Stale/missing `related`/`communities` относительно embeddings детектируются как warnings в research corpus context и viz meta (`knowledge_base.freshness`); сами `search local/global` по-прежнему не поднимают degraded только из‑за пустого графа. Провенанс проброшен из hybrid-строк.
