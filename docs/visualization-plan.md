...
**Задача:** пять валидированных агрегаций (все на реальном корпусе <2 s): (1) doc-level similarity fold (контракт №2/№3); (2) topic co-occurrence на distinct-документах (контракт №1) с порогом; (3) community rollups — label/summary/top_topics читаются из stored-узлов `communities` (single source, без пересчёта), размеры/членство — запросом; (4) таймлайн-корзины месяц×источник и месяц×топ-топики (distinct-doc, защитный FILTER null published_at + счётчик `docs_without_dates`); (5) ego-подграф одного документа (соседи + общие сущности).
