...
V5 пока находится в design phase: [Feature 007](../specs/007-writer-research-workflow/spec.md) проектирует immutable evidence dossier с chunk-level citations, curation lineage и versioned file round-trip с доверенным внешним writing-agent. `knowledge-base` не вызывает модель и не расширяет MCP write-операциями; возвращённый structured writing-output package с `output_kind=draft|summary` рассматривается как недоверенный input и проходит только integrity, citation-resolution и coverage validation, а не автоматическую factual verification. Feature не считается завершённой без четырёх записанных independent acceptance sections: dossier/citation/curation, `draft`, `summary` и privacy/path safety. Архитектурная граница зафиксирована в принятом [ADR 0010](adr/0010-adopt-provenance-gated-writer-research-file-workflow.md); соответствующего runtime-кода ещё нет.
