...
Первый production-like pipeline проектируется вокруг ArangoDB: documents/chunks, graph edges, ArangoSearch full-text и vector indexes живут в одном multi-model ядре. Полнотекстовый body/title search в `kb_text_view` двуязычный: analyzers `text_en` и `text_ru` (BM25 AQL токенизирует запрос обоими и объединяет через OR). Это снижает количество движущихся частей в v1, но сохраняет явные границы storage/search/vector/graph, чтобы позже вынести отдельный движок при bottleneck.
...
Read side не выполняет ingest/index rebuild и не меняет ArangoDB. MCP сохраняется локальным read-only интерфейсом и не получает write tools для dossier или writing artifacts. File side по умолчанию пишет только в `data/generated/research/`; explicit root вне `data/generated/` требует `--acknowledge-unsafe-output`, а symlink-компоненты запрещены независимо от acknowledgements. Directories/files публикуются атомарно с owner-only `0700`/`0600` permissions на поддерживаемых POSIX-платформах.
